...
def get_country():
    element_all = wait(browser,30).until(EC.presence_of_all_elements_located((By.XPATH, '(//ul[@id="menu--regions"])[1]//a')))
    return element_all
...
def main(jenis,email,password,countries,jumlah):
    global browser
    ...
    if "root" in jenis:
        sleep(2)
        xpath_long('//input[@value="rootUser"]')
        login_account(email,password)
    else:
        sleep(2)
        xpath_long('//input[@value="iamUser"]')
    try:
        xpath_fast("//a[text()='sign in again.']")
    except:pass
...
if __name__ == '__main__':
    ...
    for i in get_data:
        ...
        try:
            main(type_acc,email,password,countries,task_number)
        except Exception as e:
            print(e)
